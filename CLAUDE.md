...
- **Prisma Client requires an explicit driver adapter** — there is no more implicit connection from `schema.prisma`'s `url = env(...)`. Always import the shared singleton from `lib/prisma.ts` (`PrismaClient` + `PrismaPg` adapter) rather than instantiating `PrismaClient` directly. The generated client lives at `app/generated/prisma` (gitignored), not `node_modules/@prisma/client` — import types from there if you need them directly. **Don't remove the root `"postinstall": "prisma generate"` script** — since the generated client is gitignored, a fresh clone/CI/Vercel checkout has no client at all until something runs `prisma generate`; without the postinstall hook the build fails with `Module not found: Can't resolve '@/app/generated/prisma/client'` (this bit the first Vercel deploy).
